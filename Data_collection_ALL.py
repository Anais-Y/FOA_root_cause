...
def run_query_sqldb(query, conn):
    """Excecuter sql query pour une base de données
...
def WHSposition(org):
    """obtenir toutes les positions des entrpôts

    Parameters
    ----------
    org : dataframe
        un tableau contient toutes les whs(nom de whs, de ville, de pays)

    Returns
    -------
    dataframe
        un tableau contient les positions(latitude et longitude) en plus
    """
...
def get_position(part_consumed_EMEA,orgs):
    """Obtenir les entrepôts passés par une pièce et leur positions

    Parameters
    ----------
    part_consumed_EMEA : dataframe
        toutes les pièces à chercher
    orgs : dataframe
        un tableau de toutes les whs

    Returns
    -------
    liste
        pour chaque process owner, obtenir une liste contient toutes les adresses
    """
    latitudes=[]
    longitudes=[]
...
def distance_WHS(pos1, pos2):
    ...
    ----------
    pos1 : liste
        couple(liste de 2 éléments) de latitude et longitude
    pos2 : liste
        couple(liste de 2 éléments) de latitude et longitude
...
def prepareData_FOA():
    """Préparer les données de FOA

    Returns
    -------
    dataframe, dataframe
        whs liste updated et donées nétoyées
    """
...
def col_distances(adresses):
    """calculer la distance pour plusieurs pièces, retourner une liste pour chaque pièce, 
        contient la distance entre chaque deux entrepôts

    Parameters
    ----------
    adresses : liste
        chaque adress dans adresses est une liste contient toutes les positions parcourues

    Returns
    -------
    liste
        liste de deux dimension
    """
    distances=[]
    for p in adresses:
        distance_part=[]
...
def col_before_and_after(data, distances):
    """calculer la distance parcourue avant et après la consommation

    Parameters
    ----------
    data : dataframe
        le tableau contient les pièces
    distances : liste
        liste de deux dimensions, contient la liste de distance pour chaque pièce

    Returns
    -------
    liste
        liste de distance avant et après la consommation
    """
...
def search_key(key, sheet_name, file_dir): # key, sheet_name:str
    """chercher un argument dans le dossier/dans des fichiers

    Parameters
    ----------
    key : string
        argument on veut chercher
    sheet_name : string
        cet argument est dans quelle feuille
    file_dir : string
        chemin de dossier
    ...
    -------
    liste
        une liste contient l'arguemnt qu'on veut dans tous les fichiers
    """
...
def cal_delta_days(critere, sheet_name, file_dir):
    """(ne pas utiliser) pour calculer la séjour dans la dernière entrepôt
    cette fonction est remplacer par la calculation de toutes séjours dans chaque entrepôt

    Parameters
    ----------
    critere : string
        argument qui signifie les status des pièces
    sheet_name : string
        cet argument est dans quelle feuille
    file_dir : string
        chemin de dossier
    ...
    -------
    liste
        contient les jours de toutes les pièces
    """
    all_files_list = os.listdir(file_dir)
    all_keys = []
    tracing_files = [i for i in all_files_list if os.path.isfile(os.path.join(file_dir,i)) and i.startswith('tracing')]
...
def system_age(file_dir):
    ...
    -------
    liste
        systeme age calculé pour toutes les pièces
    """
    all_files_list = os.listdir(file_dir)
    tracing_files = [i for i in all_files_list if os.path.isfile(os.path.join(file_dir,i)) and i.startswith('tracing')]
    system_age = []
...
def part_birthday(file_dir):
    """Chercher le jour d'installation des pièces
    (did not use)
    ...
    -------
    liste
        first installed time calculé pour toutes les pièces
    """
    all_files_list = os.listdir(file_dir)
    tracing_files = [i for i in all_files_list if os.path.isfile(os.path.join(file_dir,i)) and i.startswith('tracing')]
    part_age = []
...
def find_WHS_changes(dates, warehouses):
    """Trouver quand il change de WHS

    Parameters
    ----------
    dates : liste/série
        transaction days
    warehouses : liste/série
        whs

    Returns
    -------
    liste
        une liste de whs, une liste de change date
    """
    changes = []
    whs = []
    prev_whs = None
...
def sejour_WHS(file_dir):
    """calculer les séjours dans chaque entrepôt
...
def failure_FOA(failure_type):
    """Calculer combien de FOA parmi les failures

    Parameters
    ----------
    failure_type : liste
        liste contient tous les failures

    Returns
    -------
    liste
        nb de foa pour chaque pièces
    """
    failure_FOA=[]
...
def analyse(col_name, df):
    """Analyser combien de fois il est passé par ce RC ou Carrier
...
def complé_system_age(df):
    """Compléter le system age par données de Service Suite car system age calculé est parfois manquant
...
def FE_exp(data):
    """Calculer l'expérience des FE (moyen si plusieurs)
...
def FOA_main():
    """Merge les données de SS et FBI
    ...
    Merge = pd.merge(data, data_SS, how='left', left_on='index', right_on='rma_no')
...
def notFOA_main():
    """Merge les données de SS et FBI pour pièce non FOA
...
def WHS_transformer(file_name):
    """Transformer la dictionnaire de séjours au 6 colonnes

    Parameters
    ----------
    file_name : string
        nom du fichier qui contient nos données
...
if __name__=='__main__':
    ...
    cols_tolabel = ['adjusted_earlylife_failure','region_description','family_name', 
    ...
    # 'failed_part_description', 'product_identifier', 'goldseal_flag','Customer Country','Customer POLE',
    # 'Activity_Month_Close_Date','min_whs','max_whs','median_whs']
    # 
    ...
    data_labeled.to_excel(f'./{key_group}/Avant_cluster{datetime.date.today()}_ALL.xlsx')