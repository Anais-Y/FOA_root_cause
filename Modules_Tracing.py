...
def trace_one_step(param, value,table, item=[], dates={}):
    ...
    if table== 'txn':
        d={'rma': 'FE Txn Group', 'lpn': 'LPN','job': 'Job Number', 'so': 'SO Number', 'sopo': 'SO PO Link', 'po':'PO Number'}
    ...
    # print(begin)
    data=open_conn_FBI()
    q=query_construction(param_value,table)
    # print(q)
    sdata=run_query_sqldb(q,data)
...
def update_dates_reduced(hist,direction,date_dict,depart_date):
    ...
    while continueiter==True and j<n:
        ...
        if 1 in [c in e.upper() for c in ['RDE REPAIR', 'GPO REPAIR']]:
...
def multipletracing(values,param,direction,savefolder):
        ...
        for value in values:
            ...
            try:
                historydata=tracing(param,value,direction)
                print('data selected')
            except:
                historydata=-1
